...
2) Привязка клиентов к конкретным билдерам (тесное связывание)
...
type PlayerInfoBuilder struct { // структура билдера, отвечающего за задание персональной информации игрока
	PlayerBuilder
}

type PlayerGunBuilder struct { // структура билдера, отвечающего за задание информации об оружии игрока
	PlayerBuilder
}

func NewPlayerBuilder() *PlayerBuilder { // конструктор создания билдера игрока (внутри создается объект Игрока с дефолтными значениями)
