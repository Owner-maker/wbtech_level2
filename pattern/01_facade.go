...
2) Есть фреймворк, позволяющий заниматься конвертацией видео, но его использование крайне неудобно и запутанно, можно создать фасад - метод convertVideo(),
...
Мой пример:
Есть структуры Loader, Conveyor и Handler (элементы условного завода), в качестве фасада будет выступать метод start() у структуры
