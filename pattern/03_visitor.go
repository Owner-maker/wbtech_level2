...
Плюсы:
1) Упрощение добавления новых операций (поддержание приниципа открытости - закрытости)
...
подсчета суммы одного заказа и его запись в файл, при добавлении данного метода в интерфейс, который имплементирует Заказ, то обратная совместимость может быть
...
type Visitor interface { // интерфейс посетителя, с методами, необходимыми для "посещения" каждого нужого объекта, при этом разных типов
