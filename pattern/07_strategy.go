...
Применимость:
1) При необходимости настройки поведения объекта под конкретный случай. Мы можем вынести поведение класса (структуры) в отдельную иерархию структур и
...
Примеры:
1) К примеру в использовании сервиса прокладывания маршрута: нам необходимо на ходу удобно подкладывать новое поведение (прокладывание маршрута) для
пользователя
...
// метод сортировки -> вызывает метод сортировки конкретной стратегии внутри структуры

func (d *Data) sort() {
	d.sortAlgorithm.sort(d)
}

// конкретные реализации стратегий сортировок
